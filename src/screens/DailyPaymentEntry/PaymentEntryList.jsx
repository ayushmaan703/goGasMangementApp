...
const PaymentEntryList = ({ navigation }) => {
    ...
    useEffect(() => {
        fetchEntries();
    }, [fromDate, toDate, adminApprovalstatus, status]);
    ...
    const totalAmount = (gasEntries || []).reduce((total, item) => total + Number(item.Amount || 0), 0);
    const cashTotal = (gasEntries || [])
        .filter((item) => String(item.paytype || "").toLowerCase() === "cash")
        .reduce((total, item) => total + Number(item.Amount || 0), 0);
    const upiTotal = (gasEntries || [])
    ...
    const filteredEntries = (gasEntries || []).filter((item) =>
        String(item.Customer || "").toLowerCase().includes(searchQuery.trim().toLowerCase())
    );
                ...
                <View style={[styles.cell, styles.amountCell]}>
                    ...
                    <View style={styles.rowActions}>
                        ...
                        {status === 0 && (
                            ...
                            >
                                <FontAwesome6 name="trash" size={11} color="#DC2626" />
                            </TouchableOpacity>
                        )}
                    </View>
                </View>
    ...
    return (
        <SafeAreaView style={styles.container}>
            <CustomNavBar
                navName="Daily Payments"
                subtitle="View your daily payment transactions"
            />

            <View style={styles.searchSection}>
                <View style={styles.searchBar}>
                    <FontAwesome6 name="magnifying-glass" size={13} color="#64748B" />
                    <TextInput
                    ...
                    {searchQuery.length > 0 && (
                        <TouchableOpacity onPress={() => setSearchQuery("")} style={styles.clearSearchBtn}>
                            <FontAwesome6 name="xmark" size={11} color="#64748B" />
                        </TouchableOpacity>
                    )}
                </View>
            </View>
                            ...
                            {filteredEntries.length > 0 ? (
                                ...
                                <View style={styles.emptyView}>
                                    <View style={styles.emptyIconWrap}>
                                        <FontAwesome6
                                            name={searchQuery ? "magnifying-glass" : "file-invoice-dollar"}
                                            size={22}
                                            color="#0D6EFD"
                                        />
                                    </View>
                                    <Text style={styles.emptyTitle}>
                                        {searchQuery ? "No Matches Found" : "No Payment Records"}
                                    </Text>
                                    <Text style={styles.emptySubtitle}>
                                        {searchQuery
                                            ? `No data found matching "${searchQuery}".`
                                            : "Transactions will display inside the ledger table."}
                                    </Text>
                                </View>
                            )}
                        </ScrollView>
